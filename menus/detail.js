...
(function () {
  ...
  const { Store, Auth, formatPrice, escapeHtml, getParam, toast, rootPath } =
    window.CafeUtils;
  ...
  (async function init() {
    ...
    const imageSrc = menu.imageUrl ? `${rp}${menu.imageUrl}` : "";
    ...
    if (!menu.soldOut) {
      ...
      bar.querySelector(".js-add-cart").addEventListener("click", async () => {
        if (!(await Auth.isLoggedIn())) {
          toast("로그인 후 이용해주세요.");
          setTimeout(() => (location.href = `${rp}my/index.html`), 800);
          return;
        }
        await Store.addToCart(menu.id, qty);
